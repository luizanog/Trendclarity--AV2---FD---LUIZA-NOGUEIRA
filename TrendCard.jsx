  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', flex:1, overflow:'hidden' }}>
      <style>{`
        ...
        .trend-card-wrap:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(108,71,255,.15)}
      ...
      <div style={{ padding:'0 20px', flexShrink:0, background:'#f8f7ff' }}>
        ...
        <div style={{ display:'flex', gap:10, marginBottom:14 }}>
          <div style={{ flex:1, display:'flex', alignItems:'center', gap:10, background:'#fff', border:'1.5px solid rgba(108,71,255,.15)', borderRadius:999, padding:'10px 16px', transition:'border .2s,box-shadow .2s' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'16px 20px 100px', scrollbarWidth:'none' }}>
        ...
        {error && (
          <div style={{ textAlign:'center', padding:'32px 16px', color:'#ef4444', fontSize:14 }}>
            <i className="ti ti-alert-circle" style={{ fontSize:32, display:'block', marginBottom:8 }}/>{error}
          </div>
        )}
        ...
        {loading ? (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            {[...Array(4)].map((_,i) => (
              <div key={i} style={{ background:'#fff', borderRadius:20, overflow:'hidden', border:'1px solid rgba(108,71,255,.08)' }}>
                <div style={{ height:140, background:'#f3f0ff', animation:'pulse 1.5s infinite' }}/>
                <div style={{ padding:'10px 12px 12px' }}>
                  <div style={{ height:10, background:'#f3f0ff', borderRadius:8, width:'50%', marginBottom:8 }}/>
                  <div style={{ height:12, background:'#f3f0ff', borderRadius:8, width:'80%', marginBottom:6 }}/>
                  <div style={{ height:12, background:'#f3f0ff', borderRadius:8, width:'60%' }}/>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'60px 16px', gap:12, textAlign:'center' }}>
            ...
            {!q && (
              <button onClick={() => navigate('/add')}
                style={{ marginTop:8, padding:'10px 24px', borderRadius:999, background:'linear-gradient(135deg,#6c47ff,#9b77ff)', color:'#fff', fontSize:14, fontWeight:600, border:'none', cursor:'pointer', display:'flex', alignItems:'center', gap:8, boxShadow:'0 4px 14px rgba(108,71,255,.30)', fontFamily:'Inter,sans-serif' }}>
      ...
      {/* FAB */}
      <button onClick={() => navigate('/add')}
        style={{ position:'absolute', bottom:88, right:20, width:52, height:52, borderRadius:'50%', background:'linear-gradient(135deg,#6c47ff,#9b77ff)', color:'#fff', fontSize:26, border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 8px 24px rgba(108,71,255,.40)', zIndex:10, transition:'transform .2s' }}
